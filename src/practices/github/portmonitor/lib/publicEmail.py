...
class Send_Email:
    def __init__(self,conf):
        self.user,self.password,self.smtp_server,self.toemail,self.xlsfile,self.change_add_list,self.change_del_list,\
            self.weakpass_result = conf['email_user'],conf['email_pass'],conf['smtp_server'],conf['target_email'], \
    ...
    def send(self):
        ...
        if self.change_add_list or self.change_del_list or self.weakpass_result:
            ...
            for result in self.change_add_list:
                ...
                add_port_info += """<tr><td>%s</td><td>%s</td><td>%s</td><td>TCP</td><td>关闭</td></tr>""" % (
                    value[0],value[1],value[2]
                )

            html_add = html_format.format(title='关闭端口服务如下：', protocol='协议', status='状态', info=add_port_info)
